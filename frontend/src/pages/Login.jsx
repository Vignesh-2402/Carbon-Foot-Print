import React, { useState } from "react";
import { Navigate } from "react-router-dom";
import { useAuth } from "../context/AuthContext";
import GoogleSignInButton from "../components/GoogleSignInButton";

export default function Login() {
  const {
    user,
    loading,
    firebaseEnabled,
    googleClientId,
    loginGoogle,
    loginEmail,
    registerEmail,
  } = useAuth();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isRegister, setIsRegister] = useState(false);
  const [error, setError] = useState("");

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-spin h-8 w-8 border-4 border-eco-500 border-t-transparent rounded-full" />
      </div>
    );
  }
  if (user) return <Navigate to="/" replace />;

  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    try {
      if (isRegister) await registerEmail(email, password);
      else await loginEmail(email, password);
    } catch (err) {
      setError(err.message);
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-eco-50 to-eco-100 p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          <span className="text-5xl">🌱</span>
          <h1 className="text-2xl font-bold mt-2">EcoTrack</h1>
          <p className="text-gray-500">Track & reduce your carbon footprint</p>
        </div>

        {/* Primary: Google Sign-In */}
        {googleClientId ? (
          <GoogleSignInButton onError={setError} />
        ) : firebaseEnabled ? (
          <button
            type="button"
            onClick={() => loginGoogle().catch((e) => setError(e.message))}
            className="w-full flex items-center justify-center gap-2 border rounded-lg py-3 hover:bg-gray-50 transition"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" aria-hidden="true">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Continue with Google
          </button>
        ) : (
          <p className="text-sm text-red-600 bg-red-50 rounded-lg p-4 text-center">
            Authentication is not configured. See{" "}
            <strong>infrastructure/FIREBASE_SETUP.md</strong> to link Firebase
            and set OAuth credentials.
          </p>
        )}

        {/* Optional: Email/Password (Firebase only) */}
        {firebaseEnabled && (
          <>
            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t" />
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="bg-white px-2 text-gray-500">or email</span>
              </div>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                type="email"
                placeholder="Email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
                className="w-full border rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-eco-500 outline-none"
                aria-label="Email"
              />
              <input
                type="password"
                placeholder="Password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                required
                minLength={6}
                className="w-full border rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-eco-500 outline-none"
                aria-label="Password"
              />
              <button
                type="submit"
                className="w-full border border-eco-600 text-eco-700 rounded-lg py-2.5 hover:bg-eco-50 font-medium"
              >
                {isRegister ? "Create Account" : "Sign In with Email"}
              </button>
            </form>
            <button
              type="button"
              onClick={() => setIsRegister(!isRegister)}
              className="w-full text-center text-sm text-eco-600 mt-3 hover:underline"
            >
              {isRegister
                ? "Already have an account? Sign in"
                : "Need an account? Register"}
            </button>
          </>
        )}

        {error && (
          <p className="text-red-600 text-sm mt-4 text-center" role="alert">
            {error}
          </p>
        )}

        <p className="text-xs text-gray-400 text-center mt-6">
          Sign in to save your carbon data and get personalized AI insights.
        </p>
      </div>
    </div>
  );
}
